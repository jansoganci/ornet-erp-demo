-- Migration: 00114_subscriptions_overdue_filter
-- Description: Add has_overdue_pending boolean to subscriptions_detail view.
--   A subscription has_overdue_pending = true when it has at least one
--   subscription_payment with status='pending' whose payment_month is before
--   the current calendar month — i.e. the invoice is at least 1 month overdue.
...
SELECT
  ...
  -- TRUE when at least one pending payment is from a past billing month
  EXISTS (
